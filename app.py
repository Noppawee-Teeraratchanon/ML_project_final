...
def export_log_to_csv(n_clicks, log_data):
    ...
    if n_clicks:
        ...
        csv_file_path = os.path.join(current_directory, 'root/log_data.csv')
...
def display_page(pathname):
    if pathname == '/':
        ...
        return html.Div([
            html.Div([
                html.P('Origin Account: the account number of origin (e.g c123445)',style={'marginBottom': '10px','margin-left': '50px'}),
                html.P('Destination Account: the account number of destination', style={'marginBottom': '10px','margin-left': '50px'}),
                html.P('New Origin balance: the balance of origin account after transaction', style={'marginBottom': '10px','margin-left': '50px'}),
                html.P('New destination balance: the balance of destination account after transaction', style={'marginBottom': '10px','margin-left': '50px'}),  
                html.P('Amount: the amount transaction in currency', style={'marginBottom': '10px','margin-left': '50px'}),   
                html.P('Transaction Type:CASH-IN, CASH-OUT, DEBIT, PAYMENT and TRANSFER', style={'marginBottom': '10px','margin-left': '50px'}),
                html.P('Origin Blacklist: origin account has blacklist flag or not', style={'marginBottom': '10px','margin-left': '50px'}),
                html.P('Destination Blacklist: destination account has blacklist flag or not', style={'marginBottom': '10px','margin-left': '50px'}),   
            ],style={'textAlign': 'left'}),

            html.Div([
                html.H6("Original Acount: ",  style={'display':'inline-block','margin-right': '10px'}),
                dcc.Input(id='origin', type='text',style={'margin-right': '50px'}),
                html.H6("Destination Acount: ",  style={'display':'inline-block', 'margin-right': '10px'}),
                dcc.Input(id='destination', type='text'),  
            ],style={'display': 'flex', 'justify-content': 'center', 'align-items': 'center', 'margin-top': '20px'}),

            html.Div([
                html.H6("New Origin balance: ",  style={'display':'inline-block','margin-right': '10px'}),
                dcc.Input(id='newbalanceOrig', type='number',style={'margin-right': '50px'}),
                html.H6("New destination balance: ",  style={'display':'inline-block', 'margin-right': '10px'}),
                dcc.Input(id='newbalanceDest', type='number'),  
            ],style={'display': 'flex', 'justify-content': 'center', 'align-items': 'center', 'margin-top': '50px'}),


            html.Div([
                html.H6("Amount: ",  style={'display':'inline-block','margin-right': '10px'}),
                dcc.Input(id='Amount', type='number',style={'margin-right': '50px'}),
                html.H6("Transaction Type: ",  style={'display':'inline-block', 'margin-right': '10px'}),
                dcc.Dropdown(
                    id='Transaction_Type',
                    options=[
            ...
            html.Div([
                html.H6("Origin Blacklist: ",  style={'display':'inline-block','margin-right': '10px'}),
                dcc.Dropdown(
                ...
                html.H6("Destination Blacklist: ",  style={'display':'inline-block', 'margin-right': '10px'}),
                dcc.Dropdown(
            ...
            ], style={'display': 'flex', 'flex-direction': 'column', 'align-items': 'center', 'justify-content': 'center', 'height': '10vh','margin-top': '20px'}),

            html.H5(id='output-container', style={'color':'Red', 'text-align': 'center', 'margin-top': '20px'}),
...
@app.callback(
    ...
    [
        ...
        State('blacklist_flag_org', 'value'),
        State('blacklist_flag_dest', 'value'),
    ]
)

def update_output(n_clicks, origin, destination, Amount, newbalanceOrig, newbalanceDest, Transaction_Type, blacklist_flag_org, blacklist_flag_dest):
    global log_df
    if n_clicks > 0:
        input_values = [origin, destination,Amount, newbalanceOrig, newbalanceDest, Transaction_Type, blacklist_flag_org, blacklist_flag_dest]
        if any(value is None for value in input_values):
            ...
            scaler = pickle.load(open('/root/scaler.pkl', 'rb'))
            [Amount_scale,  newbalanceOrig_scale, newbalanceDest_scale]  = scaler.transform([[Amount,  newbalanceOrig, newbalanceDest]])[0]
        
        features = np.array([[Amount_scale, newbalanceOrig_scale, newbalanceDest_scale] + transaction_type_encoded + [blacklist_flag_org, blacklist_flag_dest]])
        ...
        if log_df is None or log_df.empty:
            log_df = pd.DataFrame(columns=['origin','destination','Amount', 'newbalanceOrig', 'newbalanceDest', 'Transaction_Type', 'blacklist_flag_org', 'blacklist_flag_dest', 'Prediction'])
        ...
        return html.Div(f'Predicted value: {result}'), log_table_data

    else:
        return html.Div('Please enter values and click to predict'), dash.no_update
